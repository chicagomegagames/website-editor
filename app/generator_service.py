...
class GeneratorService(object):
    ...
    def deploy(self, theme_path = None, image_service = None):
        ...
        directory = os.path.join(self.location, directory_name)
        os.mkdir(directory)
        ...
        try:
            ...
            for page in pages_to_generate:
                ...
                output_dir = os.path.join(directory, slug)
                output_file = os.path.join(output_dir, "index.html")
                make_directory_tree(output_dir)

                template = template_environment.get_template("page.html")
                with open(output_file, "w+") as writer:
                    writer.write(template.render(page=page))

            assets_dir = os.path.join(theme_path, "assets")
            if os.path.exists(assets_dir):
                shutil.copytree(assets_dir, os.path.join(directory, "assets"))

            if image_service:
                shutil.copytree(image_service.upload_path, os.path.join(directory, "images"))
